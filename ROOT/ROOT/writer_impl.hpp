...
template <>
void Writer<Eigen::MatrixX2d>::writing_process(std::string filename) {
    print_final_result();
...
void Writer<T>::choose_how(std::unique_ptr<Printer<V>>& printer, std::string filename) {
    if (method == CONSOLE) {
        ...
        if (method == CSV) {
            ...
            std::cin >> sep;
            printer = std::make_unique<csvPrinter<V>>(filename, sep, ow_choice);
        } else if (method == DAT) {
            printer = std::make_unique<datPrinter<V>>(filename, ow_choice);
        } else {
            printer = std::make_unique<GnuplotPrinter<V>>(filename, ow_choice);
        }
    }
}
